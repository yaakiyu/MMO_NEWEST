...
|:-----------|:------------:| ------------ |:-----------|:------------:| 
| "Python" | 3.10.9 | | "discord.py" | 2.0.1 |
| "pip" | 20.0.2 | | "aiosqlite" | 0.17.0 |

```markdown
書き方はdiscord.pyの2.0でcogsを使用しています。
cogsとは: https://qiita.com/kymybk/items/14d5d10f33315c8a9aee
```
# このリニューアルコードについて:
```
yaakiyuです。このコードが最新版のdiscord.pyで動作するように改良しました。
ソースコードに点在する初心者の理解を阻むような部分も編集を加えていますが、動作は一切変わりません。
なお動作確認は行なっていないため何か問題がありましたらdiscordのyaakiyu#4052まで。
その他の変更点：
・動作環境を修正、あとpipでインストールする際に必要のないdependencyは削除
・必要のないf-stringsを削除
以下、フォーク元の文章そのままです。
```
```
